...
function reanimateColors() {
    ...
    animatedColor.red_orange = blinker ? color.red : "#E7896D";
}
function getColor(colorNumber) {
    switch (colorNumber) {
        ...
        case "tree":
            return "#267524";
        case 100:
            return animatedColor.red_orange;
        case 101:
...
function drawGuiRoundRect(x, y, length, height, stroke = false, radius = 5, fill = true) {
...
//context, x, y, length, height, aspect, angle, skin, borderless, fill
function drawTrapezoid(context, x, y, length, height, aspect, angle, skin, borderless, fill) {
    let h = [];
    ...
    context.lineTo(x + l[0] * Math.cos(angle - r[0]), y + l[0] * Math.sin(angle - r[0]));
        break;
        case 8: // Split Barrel without spike
    context.lineTo(x + l[0] * Math.cos(angle + r[0]), y + l[0] * Math.sin(angle + r[0]));
    context.lineTo(x + l[1] * Math.cos(angle + Math.PI - r[1]), y + l[1] * Math.sin(angle + Math.PI - r[1]));
    context.lineTo(x + l[1] * Math.cos(angle + Math.PI + r[1]), y + l[1] * Math.sin(angle + Math.PI + r[1]));
    context.lineTo(x + l[0] * Math.cos(angle - r[0]), y + l[0] * Math.sin(angle - r[0]));
...
/*function drawTrapezoid(context, x, y, length, height, aspect, angle, skin, borderless, fill) {
    ...
    context.beginPath();
    switch (skin) {
        case 1: // Invisa-barrel (Pretty simple)
            context.closePath();
            break;
        case 2: // L Coilgun Barrel
            context.lineTo(x + l[0] * Math.cos(angle + r[0]), y + l[0] * Math.sin(angle + r[0]));
            context.lineTo(x + l[1] * Math.cos(angle + Math.PI - r[1]), y + l[1] * Math.sin(angle + Math.PI - r[1]));
...
//context, x, y, length, height, aspect, angle, skin, borderless, fill
            drawTrapezoid(context, xx + drawSize * gx, yy + drawSize * gy, drawSize * (g.length / 2 - (g.aspect === 1 ? position * 2 : 0)), (drawSize * g.width) / 2, g.aspect, g.angle + rot, g.skin, borderless, fill);
        }
    }
    // Draw body
...
/*() {
    ...
    for (let { x, y, colorIndex, index } of tiles) {
        ...
        ctx.lineWidth = 2;
        drawGuiRect(ax, ay, size, size, true);
        //drawText(picture.name, x + ((upgradeKey ? 0.9 : 1) * len) / 2, y + height - 6, height / 8 - 3, color.white, "center");
    }
}*/
...
function drawSkillBars(spacing, alcoveSize) {
    ...
    for (let i = 0; i < gui.skills.length; i++) {
        ...
        // Skill value
        if (level) {
            drawText(textcolor === col ? "MX" : "+" + level, Math.round(x + len + 4) + 0.5, y + height / 2, height - 5, col, "left", true);
        }
...
function drawSelfInfo(spacing, alcoveSize, max) {
    ...
    // Draw the class type
    drawText("Ranking - " + gui.__s.getLevel() + " " + global.mockups[gui.type].name, x + len / 2, y + height / 2, height - 4, color.orange, "center", true);
    height = 14;
    y -= height + vspacing;
...
function drawMinimapAndDebug(spacing, alcoveSize) {
    ...
    // Text
    if (global.showDebug) {
        drawText("Planar.io", x + len, y - 50 - 5 * 14 - 2, 15, animatedColor.gay, "right");
        ...
        drawText((100 * gui.fps).toFixed(2) + "% : " + global.metrics.rendertime + " FPS", x + len, y - 50 - 1 * 14, 10, global.metrics.rendertime > 10 ? color.yellow : color.orange, "right");
        drawText(global.metrics.latency + " ms - " + global.serverName, x + len, y - 50, 10, color.yellow, "right");
    } else {
        drawText("Planar.io", x + len, y - 50 - 2 * 14 - 2, 15, animatedColor.gay, "right");
        drawText((100 * gui.fps).toFixed(2) + "% : " + global.metrics.rendertime + " FPS", x + len, y - 50 - 1 * 14, 10, global.metrics.rendertime > 10 ? color.yellow : color.orange, "right");
        drawText(global.metrics.latency + " ms : " + global.metrics.updatetime + "Hz", x + len, y - 50, 10, color.yellowe, "right");
    }
...
function drawAvailableUpgrades(spacing, alcoveSize) {
    ...
    if (gui.upgrades.length > 0) {
        ...
        for (let i = 0; i < gui.upgrades.length; i++) {
            ...
            // Upgrade key
            if (upgradeKey) {
                drawText("[" + upgradeKey + "]", x + len - 4, y + height - 6, height / 8 - 3, color.white, "right");
            }
            ctx.strokeStyle = color.black;
            ctx.globalAlpha = 1;
            ctx.lineWidth = 3;
...
const gameDraw = (ratio, drawRatio) => {
    ...
    if (global.showTree) {
        ...
        drawSkillBars(spacing, alcoveSize * 0.95);
...
const gameDrawDead = () => {
    ...
    drawText("Game over man, game over.", x, y - 80, 8, color.red, "center");
    drawText("Rank " + gui.__s.getLevel() + " " + global.mockups[gui.type].name, x - 170, y - 30, 24, color.orange);
    drawText("Final score: " + util.formatLargeNumber(Math.round(global.finalScore.get())), x - 170, y + 25, 50, color.yellow);
    drawText("⌚ Survived for " + util.timeForHumans(Math.round(global.finalLifetime.get())), x - 170, y + 55, 16, color.green);
