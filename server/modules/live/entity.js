let EventEmitter = require('events'),
    ...
    function setNatural(natural, type) {
        ...
        function recursiveSet(natural, type) {
            if (type.PARENT) {
                type.PARENT.forEach(parent => recursiveSet(natural, parent));
            }
    
            if (type.BODY) {
                Object.assign(natural, type.BODY);
            }
        }
    
        recursiveSet(natural, type);
    }
...
class Entity extends EventEmitter {
            ...
            case "splitterSentrySwarm":
                this.onDead = () => {
                    ...
                    for (let i = 0; i < 4; i++) {
                        ...
                        shape.team = -100;
                        shape.define(Class.sentrySwarm);
    ...
    refreshBodyAttributes() {
        ...
        this.fov = fovMultiplier * this.FOV * 275 * Math.sqrt(this.size)// * (1 + 0.003 * this.level);
            ...
            case "gravityB":
                ...
                case "gravityE":
                    ...
                    case "draggingA":
                        ...
                        break;
                    case "draggingB":
                        this.velocity.x -= a / 1.45 * Math.sin(2 * Math.PI / 4);
                        this.velocity.y += a / 1.45 * Math.cos(2 * Math.PI / 4);
                        this.damp = -.00125;
                        this.topSpeed = 70;
                        break;
                    case "draggingC":
                        this.velocity.x -= a / 1.45 * Math.sin(3 * Math.PI / 4);
                        this.velocity.y += a / 1.45 * Math.cos(3 * Math.PI / 4);
                        this.damp = -.00125;
                        this.topSpeed = 70;
                        break;
                    case "draggingD":
                        this.velocity.x -= a / 1.45 * Math.sin(4 * Math.PI / 4);
                        this.velocity.y += a / 1.45 * Math.cos(4 * Math.PI / 4);
                        this.damp = -.00125;
                        this.topSpeed = 70;
                        break;
                    case "draggingE":
                        this.velocity.x -= a / 1.45 * Math.sin(5 * Math.PI / 4);
                        this.velocity.y += a / 1.45 * Math.cos(5 * Math.PI / 4);
                        this.damp = -.00125;
                        this.topSpeed = 70;
                        break;
            case 'assembler':
