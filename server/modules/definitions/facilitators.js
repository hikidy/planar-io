...
exports.makeNecro = (type, name = -1) => {
    let output = exports.dereference(type);
    let spawner = {
        /********* LENGTH    WIDTH     ASPECT        X             Y         ANGLE     DELAY */
        POSITION: [6, 12, 1.2, 8, 0, 180, 0],
        PROPERTIES: {
            SHOOT_SETTINGS: exports.combineStats([g.drone, g.sunchip, g.weak]),
            TYPE: ["sunchip", { INDEPENDENT: true }],
            AUTOFIRE: true,
            SYNCS_SKILLS: true,
    ...
    output.GUNS = type.GUNS == null ? [spawner] : type.GUNS.concat([spawner]);
    output.SHAPE = 4;
    output.LABEL = name == -1 ? "Under-" + type.LABEL : name;
    return output;
}
...
exports.makeRevolution = (type, name = -1, options = {}) => {
    if (options.type != null) {
        turret.type = options.type;
    }
    if (options.size != null) {
    ...
    if (type.TURRETS == null) {
        output.TURRETS = [ring, babies];
    } else {
        output.TURRETS = [...type.TURRETS, ring, babies];
    }
    if (name == -1) {
        output.LABEL = "Auto-" + type.LABEL;
    } else {
...
exports.neoMakeAnimTank = (exportName, tankName, tanks, options = {}) => {
    let animLength = options.animLength || 31,
        hasProp = options.hasProp == undefined ? true : options.hasProp,
        duration = options.duration || 20,
        tankInfo = options.tankInfo || {};
